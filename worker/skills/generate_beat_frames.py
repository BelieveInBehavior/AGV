...
⚠️ 姿态定格铁律：scene_prompt 描述的是一张静态照片，图像生成模型无法理解动态语义（"拉回""推开""扑向""甩开""冲进"等动词）。必须将所有动作过程翻译为该动作的**结果姿态**——即动作完成瞬间的身体空间关系、肢体位置和接触点。写 scene_prompt 时自问：「如果我给摄影师看这段文字，他能摆出这个pose吗？」如果需要理解因果链才能还原姿态，说明还不够具体
...
G9. 动作→姿态转换（静态照片原则）：scene_prompt 的消费者是图像生成模型，它只能渲染静态画面。所有带运动语义的动词必须转写为可直接摆拍的「结果姿态」：写明身体部位的空间位置、接触面、角度和重心。
