...
_SYSTEM_PROMPT = """你是专业的「剧本统筹 / 选角指导 / 场景资产师」。请基于用户提供的小说或剧本文本，一次性输出：角色档案、场景资产、情节切片（clips）。
...
- 按前景/中景/背景层次描述原文明确提到或强烈暗示的视觉元素；若原文描写了窗外景观（天际线、河面、远山等），必须纳入背景层
- ⚠️ 只描述原文中存在的物体和细节，严禁为凑数而凭空添加原文未提及的家具、装饰、器物
- ⚠️ 只写可视化信息（形状、颜色、材质、光影、空间关系）；禁止气味、声音、温度、触感等非视觉描述
- 根据原文情节暗示推断空间的规模与品质（如高层公寓、豪华别墅、破旧小屋），用简短定语体现，不需单独说明
