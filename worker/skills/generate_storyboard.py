...
【运镜 cameraMovement】
可使用单一运镜（固定、左摇、右摇、上摇、下摇、推轨、拉轨、升降、跟拍、手持、变焦）或复合运镜序列（用「→」连接，如「特写推轨→迅速拉远→环绕旋转」）。复合运镜应体现镜头叙事节奏，如"从细节到全局""跟随动作""情绪强调"。

【videoPrompt 运镜叙事要求】
videoPrompt 必须包含：
1. 起始构图与景别（如「镜头从人物踉跄的特写开始」）
2. 运动方式与节奏（如「随动作迅速拉远」「缓慢环绕旋转」）
3. 落幅焦点与情绪意图（如「最终聚焦在颤抖的肩膀，强调柔弱无助」）
说话镜头须写「正在说话」；用角色名指代角色，勿用年龄段+性别替代。
...
[
  {
    ...
    "location": "场景名",
    "shotType": "景别",
    "cameraMovement": "单一或复合运镜，如「特写→拉远→环绕」",
    "mood": "情绪基调",
    "action": "正在发生什么",
    "dialogue": "本镜台词或旁白，无则空字符串",
    "imagePrompt": "中文 AI 生图提示词：含画风、构图、光线、角色基础形象+本镜状态，遵守 G1–G4",
    "videoPrompt": "中文视频动态描述：起始构图→运动方式与节奏→落幅焦点与情绪意图；说话镜头须写「正在说话」；用角色名指代角色，勿用年龄段+性别替代"
  }
]"""
