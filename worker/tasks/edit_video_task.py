...
流程:
  ...
  4. 上传处理后的视频到 OSS
  5. 写回 clip.editedVideoUrl 或创建剧集级 compiledVideoUrl
"""
...
def _merge_clips(
    ...
    if len(input_paths) == 1:
        ...
        if proc.returncode != 0:
            detail = (proc.stderr or proc.stdout or '').strip()[-500:]
            raise RuntimeError(f'ffmpeg 视频转码失败: {detail or "unknown error"}')
        return
                ...
                try:
                    ...
                    try:
                        ...
                        if remove_vocals:
                            ...
                            _remove_vocals(input_path, output_path, vocal_strength)
                        else:
                            # 只转码，不去人声
                            cmd = [
