...
import { backfillClipCharacters } from '../utils/clip-characters.js';
import { safeParseJson } from '../utils/json-utils.js';
import { chatCompletionText } from '../utils/llm-openai-compat.js';

const SYSTEM_PROMPT = `你是专业的「剧本统筹 / 选角指导 / 场景资产师」。请基于用户提供的小说或剧本文本，一次性输出：角色档案、场景资产、情节切片。
【总规则】
...
export async function analyzeStorySkill({ text, language = 'zh' }) {
  ...
  const clips = backfillClipCharacters(result.clips || []);
